...
export class EditarEspacioDialogComponent {
  ...
  iniciarFormEspacio() {
    this.formEspacio = this._formBuilder.group({
      ...
      horas: [
        '',
        [Validators.required, this.horaValidador(this.infoEspacio.horaFormato)],
      ],
    });
  ...
  cargarDatosFormEspacio() {
    const facultad = this.facultades.find(
      (facultad: any) => facultad.Id === this.infoEspacio.sede.Id
    );
    ...
    const edificio = this.bloques.find(
      (bloque: any) => bloque.Id === this.infoEspacio.edificio.Id
    );
    this.formEspacio.patchValue({
      bloque: edificio,
    });
    this.cargarSalonesSegunBloque(edificio);
    const salon = this.salones.find(
      (salon: any) => salon.Id === this.infoEspacio.salon.Id
    );
    this.formEspacio.patchValue({
      salon: salon,
    });
  ...
  horaValidador(intervalo: string, maxHoraPermitida: number = 23): ValidatorFn {
    return (control: AbstractControl): ValidationErrors | null => {
      ...
      const horasRestantes =
        maxHoraPermitida - (horaInicioH + minutoInicioM / 60);
